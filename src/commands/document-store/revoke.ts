...
export const describe = 'Revokes a hash from a document store deployed on the blockchain';
...
// Revoke the token with the provided inputs
export const revokeToken = async ({
  ...
  try {
    info(`Revoking ${documentHash} from the document store ${documentStoreAddress}`);
    ...
    const shouldProceed = await performDryRunWithConfirmation({
      ...
      getTransactionCallback: async () => {
        ...
        const tx = await documentStore['revoke(bytes32)'].populateTransaction(documentHash);
    ...
    if (canEstimateGasPrice(network)) {
      ...
      transaction = await documentStoreRevoke(documentStoreAddress, documentHash, wallet, {
    ...
    success(`Token with hash ${documentHash} has been revoked from ${documentStoreAddress}`);
