...
/** Prompt for all inputs needed to gaslessly reject a pending holder transfer. */
export const promptForGaslessRejectTransferHolderInputs =
...
/**
 * Runs a gasless reject-transfer-holder transaction end to end: resolves the title escrow,
 * recovers the caller's raw private key, checks the account is eligible for sponsorship, then
 * submits the sponsored UserOperation via @trustvc/trustvc's `rejectTransferHolderGasless`.
 */
export const runRejectTransferHolderGasless = async (
  ...
  try {
    ...
    // rejectTransferHolderGasless() re-derives and encrypts the remark itself from the raw string.
    ...
    // Rejecting a holder transfer requires the caller to currently be the holder.
    ...
    info(
      `Submitting gasless rejection of the pending holder transfer for tokenId ${args.tokenId}. Gas is sponsored by the PlatformPaymaster — no ETH is required from your wallet.`,
    );

    const transactionHash = await rejectTransferHolderGasless(
      { titleEscrowAddress },
      smartAccountClient,
      { remarks: args.remark },
    ...
    success(
      `Transferable record with tokenId ${args.tokenId}'s holder transfer has been successfully rejected to previous holder`,
    );
