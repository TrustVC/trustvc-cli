...
export const describe =
  'Unwrap wrapped OpenAttestation document(s) (Only for individual documents)';
...
export const promptForInputs = async (): Promise<UnwrapOAInput> => {
  ...
  const pathToOutputDirectory = await input({
    message:
      'Please enter the directory to save the unwrapped OpenAttestation document(s) (optional):',
    required: false,
    default: '.',
  });
...
export const unwrapOA = async ({ docPaths, pathToOutputDirectory }: UnwrapOAInput) => {
