...
import { CHAIN_ID, rejectTransferOwners as rejectTransferOwnersImpl } from '@trustvc/trustvc';
...
export const builder = (yargs: Argv): Argv =>
  yargs.option('gasless', {
    ...
    default: false,
    description:
      'Perform this rejection as a gasless (EIP-7702 + Pimlico sponsored) transaction instead of paying gas directly. Requires PIMLICO_API_KEY and an EIP7702 implementation address to be set in the environment; you will be prompted for the PlatformPaymaster address.',
  });

export const handler = async (argv: { gasless?: boolean } = {}): Promise<string | undefined> => {
  try {
    if (argv.gasless) {
      const gaslessAnswers = await promptForGaslessRejectTransferOwnerHolderInputs();
    ...
    await rejectTransferOwnerHolderHandler(answers);
  } catch (err: unknown) {
...
// Reject the transfer of owner and holder with the provided inputs
export const rejectTransferOwnerHolderHandler = async (args: TitleEscrowRejectTransferCommand) => {
  try {
    info(
      `Connecting to the registry ${args.tokenRegistryAddress} and attempting to reject the change of owner and holder of the transferable record ${args.tokenId} to previous owner and holder`,
    );
    warn(
...
/**
 * Rejects both owner (beneficiary) and holder transfers, reverting to previous owners.
 * This operation cancels a pending transfer of both beneficiary and holder.
 *
...
export const rejectTransferOwnerHolder = async ({
  ...
  const shouldProceed = await performDryRunWithConfirmation({
    ...
    getTransactionCallback: async () => {
      ...
      // Validate that both previous holder and beneficiary exist for rejection
      ...
      const tx = await titleEscrow.rejectTransferOwners.populateTransaction(encryptedRemark);
  ...
  if (canEstimateGasPrice(network)) {
    ...
    // Execute reject transfer owners with EIP-1559 gas parameters
    transaction = await rejectTransferOwnersImpl(
    ...
    // Execute reject transfer owners without gas estimation (for networks that don't support it)
    transaction = await rejectTransferOwnersImpl(
      { tokenRegistryAddress, tokenId },
      wallet,
      { remarks: remark },
