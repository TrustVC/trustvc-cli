...
import { promptForGaslessTransferHolderInputs, runTransferHolderGasless } from '../gasless';
...
export const handler = async (argv: { gasless?: boolean } = {}): Promise<string | undefined> => {
  try {
    if (argv.gasless) {
      const gaslessAnswers = await promptForGaslessTransferHolderInputs();
      return await runTransferHolderGasless(gaslessAnswers);
    }
