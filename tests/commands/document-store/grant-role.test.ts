...
import {
  ...
  promptForInputs,
} from '../../../src/commands/document-store/grant-role';
...
describe('document-store/grant-role', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetAllMocks();
  });
  ...
  describe('grantRoleToDocumentStore', () => {
    let documentStoreGrantRoleMock: MockedFunction<any>;
    ...
    beforeEach(async () => {
      ...
      documentStoreGrantRoleMock = trustvcModule.documentStoreGrantRole as MockedFunction<any>;
    ...
    it('should successfully grant ISSUER_ROLE and display transaction details', async () => {
      const mockArgs: any = {
        documentStoreAddress: '0x1234567890123456789012345678901234567890',
        role: 'ISSUER_ROLE',
        account: '0x9876543210987654321098765432109876543210',
      ...
      documentStoreGrantRoleMock.mockResolvedValue(mockTransaction);
      waitForTransactionMock.mockResolvedValue({
        transactionHash: '0xtxhash123',
        blockNumber: 12345,
      });

      const result = await grantRoleToDocumentStore(mockArgs);

      expect(documentStoreGrantRoleMock).toHaveBeenCalled();
      expect(result).toBe(mockArgs.documentStoreAddress);
    });

    it('should handle errors during grant role execution', async () => {
      const mockArgs: any = {
        documentStoreAddress: '0x1234567890123456789012345678901234567890',
        role: 'ISSUER_ROLE',
        account: '0x9876543210987654321098765432109876543210',
      ...
      documentStoreGrantRoleMock.mockRejectedValue(new Error(errorMessage));
