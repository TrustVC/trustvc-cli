...
import { handler, issueToken, promptForInputs } from '../../../src/commands/document-store/issue';
...
describe('document-store/issue', () => {
  ...
  describe('issueToken', () => {
    let documentStoreIssueMock: MockedFunction<any>;
    ...
    beforeEach(async () => {
      ...
      documentStoreIssueMock = trustvcModule.documentStoreIssue as MockedFunction<any>;
    ...
    it('should successfully issue token and display transaction details', async () => {
      const mockArgs: any = {
        documentStoreAddress: '0x1234567890123456789012345678901234567890',
        documentHash: '0xabcdef1234567890',
        encryptedWalletPath: './wallet.json',
      ...
      documentStoreIssueMock.mockResolvedValue({
        hash: mockTransaction.transactionHash,
        wait: vi.fn().mockResolvedValue(mockTransaction),
      });

      const result = await documentStoreIssueMock(mockArgs);

      expect(documentStoreIssueMock).toHaveBeenCalledWith({
    ...
    it('should handle errors during issue', async () => {
      const mockArgs: any = {
        documentStoreAddress: '0x1234567890123456789012345678901234567890',
        documentHash: '0xabcdef1234567890',
        encryptedWalletPath: './wallet.json',
      ...
      documentStoreIssueMock.mockRejectedValue(new Error(errorMessage));

      const result = await issueToken(mockArgs);

      expect(result).toBeUndefined();
      expect(documentStoreIssueMock).toHaveBeenCalled();
    });
  });
  ...
  describe('handler', () => {
    ...
    it('should successfully execute the complete issue flow', async () => {
      ...
      const trustvcModule = await import('@trustvc/trustvc');
      const issueMock = trustvcModule.documentStoreIssue as MockedFunction<any>;
