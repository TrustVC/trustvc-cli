...
import { handler } from '../../../../src/commands/gasless/admin/add-authorized-caller';
...
describe('gasless/admin/add-authorized-caller', () => {
  const base = {
    network: 'sepolia',
  ...
  it('should authorize the prompted caller against the prompted paymaster', async () => {
    ...
    (trustvc.addAuthorizedCaller as MockedFunction<any>).mockResolvedValue('0xtxhash');

    const result = await handler();

    expect(utils.promptAddress).toHaveBeenCalledWith(
      'caller',
      'address to authorize on the paymaster',
    );
    expect(trustvc.addAuthorizedCaller).toHaveBeenCalledWith(
      mockWallet,
    ...
    expect(common.runPaymasterAdminAction).toHaveBeenCalledWith(
      expect.objectContaining({
        ...base,
        actionLabel: `Authorizing caller ${callerAddress}`,
