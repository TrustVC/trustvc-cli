...
import {
  promptForGaslessRejectReturnToIssuerInputs,
  runRejectReturnToIssuerGasless,
} from '../../../../src/commands/gasless/title-escrow/reject-return-to-issuer';
...
describe('gasless/title-escrow/reject-return-to-issuer', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('promptForGaslessRejectReturnToIssuerInputs', () => {
    it('should return correct answers for valid inputs', async () => {
      ...
      const result = await promptForGaslessRejectReturnToIssuerInputs();
    ...
    it('should reject when network is not gasless-supported', async () => {
      ...
      await expect(promptForGaslessRejectReturnToIssuerInputs()).rejects.toThrow(
        'Unsupported network',
      );
    });
  });

  describe('runRejectReturnToIssuerGasless', () => {
    let rejectReturnedGaslessMock: MockedFunction<any>;
    ...
    beforeEach(async () => {
      ...
      rejectReturnedGaslessMock = trustvcModule.rejectReturnedGasless as MockedFunction<any>;
    ...
    it('should successfully run the gasless reject return to issuer flow', async () => {
      prepareGaslessRegistryRunMock.mockResolvedValue({
        network: 'sepolia',
        callerAddress: '0xCaller',
        smartAccountClient: { id: 'client' },
      });
      rejectReturnedGaslessMock.mockResolvedValue('0xtxhash');

      const result = await runRejectReturnToIssuerGasless(baseArgs);
      ...
      expect(rejectReturnedGaslessMock).toHaveBeenCalledWith(
    ...
    it('should return undefined and log error message when the gasless call rejects with a non-Error', async () => {
      ...
      rejectReturnedGaslessMock.mockRejectedValue('String error message');
