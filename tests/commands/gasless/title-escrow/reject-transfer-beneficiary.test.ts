...
import {
  promptForGaslessRejectTransferOwnerInputs,
  runRejectTransferOwnerGasless,
} from '../../../../src/commands/gasless/title-escrow/reject-transfer-beneficiary';
...
describe('gasless/title-escrow/reject-transfer-beneficiary', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('promptForGaslessRejectTransferOwnerInputs', () => {
    it('should return correct answers for valid inputs', async () => {
      ...
      const result = await promptForGaslessRejectTransferOwnerInputs();
    ...
    it('should reject when network is not gasless-supported', async () => {
      ...
      await expect(promptForGaslessRejectTransferOwnerInputs()).rejects.toThrow(
        'Unsupported network',
      );
    });
  });

  describe('runRejectTransferOwnerGasless', () => {
    let rejectTransferBeneficiaryGaslessMock: MockedFunction<any>;
    ...
    beforeEach(async () => {
      ...
      rejectTransferBeneficiaryGaslessMock =
        trustvcModule.rejectTransferBeneficiaryGasless as MockedFunction<any>;
    ...
    it('should successfully run the gasless reject transfer owner flow', async () => {
      ...
      rejectTransferBeneficiaryGaslessMock.mockResolvedValue('0xtxhash');

      const result = await runRejectTransferOwnerGasless(baseArgs);
      ...
      expect(rejectTransferBeneficiaryGaslessMock).toHaveBeenCalledWith(
        { titleEscrowAddress: '0xTitleEscrow' },
        { id: 'client' },
    ...
    it('should return undefined and log error message when prepareGaslessRun rejects with an Error', async () => {
      prepareGaslessRunMock.mockRejectedValue(new Error('not the current beneficiary'));
    ...
    it('should return undefined and log error message when the gasless call rejects with a non-Error', async () => {
      ...
      rejectTransferBeneficiaryGaslessMock.mockRejectedValue('String error message');
