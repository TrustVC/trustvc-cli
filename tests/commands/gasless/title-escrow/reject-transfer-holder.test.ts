...
import {
  promptForGaslessRejectTransferHolderInputs,
  runRejectTransferHolderGasless,
} from '../../../../src/commands/gasless/title-escrow/reject-transfer-holder';
...
describe('gasless/title-escrow/reject-transfer-holder', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('promptForGaslessRejectTransferHolderInputs', () => {
    it('should return correct answers for valid inputs', async () => {
      ...
      const result = await promptForGaslessRejectTransferHolderInputs();
    ...
    it('should reject when network is not gasless-supported', async () => {
      ...
      await expect(promptForGaslessRejectTransferHolderInputs()).rejects.toThrow(
        'Unsupported network',
      );
    });
  });

  describe('runRejectTransferHolderGasless', () => {
    let rejectTransferHolderGaslessMock: MockedFunction<any>;
    ...
    beforeEach(async () => {
      ...
      rejectTransferHolderGaslessMock =
        trustvcModule.rejectTransferHolderGasless as MockedFunction<any>;
    ...
    it('should successfully run the gasless reject transfer holder flow', async () => {
      ...
      rejectTransferHolderGaslessMock.mockResolvedValue('0xtxhash');

      const result = await runRejectTransferHolderGasless(baseArgs);
      ...
      expect(rejectTransferHolderGaslessMock).toHaveBeenCalledWith(
        { titleEscrowAddress: '0xTitleEscrow' },
        { id: 'client' },
    ...
    it('should return undefined and log error message when prepareGaslessRun rejects with an Error', async () => {
      prepareGaslessRunMock.mockRejectedValue(new Error('not the current holder'));

      const result = await runRejectTransferHolderGasless(baseArgs);

      const signaleModule = await import('signale');
      expect(signaleModule.error).toHaveBeenCalledWith('not the current holder');
    ...
    it('should return undefined and log error message when the gasless call rejects with a non-Error', async () => {
      ...
      rejectTransferHolderGaslessMock.mockRejectedValue('String error message');
