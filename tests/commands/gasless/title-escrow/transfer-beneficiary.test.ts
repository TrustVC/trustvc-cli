...
import {
  promptForGaslessTransferBeneficiaryInputs,
  runTransferBeneficiaryGasless,
} from '../../../../src/commands/gasless/title-escrow/transfer-beneficiary';
...
describe('gasless/title-escrow/transfer-beneficiary', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('promptForGaslessTransferBeneficiaryInputs', () => {
    it('returns correctly mapped answers for valid inputs', async () => {
      ...
      const result = await promptForGaslessTransferBeneficiaryInputs();
    ...
    it('rejects when the resolved network is not gasless-supported', async () => {
      ...
      await expect(promptForGaslessTransferBeneficiaryInputs()).rejects.toThrow(
  ...
  describe('runTransferBeneficiaryGasless', () => {
    const baseArgs = {
      network: 'sepolia',
    ...
    it('runs the gasless transfer and returns the transaction hash', async () => {
      ...
      (trustvc.transferBeneficiaryGasless as MockedFunction<any>).mockResolvedValue('0xtxhash');

      const result = await runTransferBeneficiaryGasless(baseArgs);
      ...
      expect(trustvc.transferBeneficiaryGasless).toHaveBeenCalledWith(
        { titleEscrowAddress: '0xescrow' },
        smartAccountClient,
    ...
    it('returns undefined and logs the raw value when the gasless call rejects with a non-Error', async () => {
      ...
      (trustvc.transferBeneficiaryGasless as MockedFunction<any>).mockRejectedValue('raw failure');
