...
import {
  promptForGaslessTransferHolderInputs,
  runTransferHolderGasless,
} from '../../../../src/commands/gasless/title-escrow/transfer-holder';
...
describe('gasless/title-escrow/transfer-holder', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('promptForGaslessTransferHolderInputs', () => {
    it('returns correctly mapped answers for valid inputs', async () => {
      ...
      const result = await promptForGaslessTransferHolderInputs();
    ...
    it('rejects when the resolved network is not gasless-supported', async () => {
      ...
      await expect(promptForGaslessTransferHolderInputs()).rejects.toThrow(
  ...
  describe('runTransferHolderGasless', () => {
    const baseArgs = {
      network: 'sepolia',
    ...
    it('runs the gasless transfer and returns the transaction hash', async () => {
      ...
      (trustvc.transferHolderGasless as MockedFunction<any>).mockResolvedValue('0xtxhash');

      const result = await runTransferHolderGasless(baseArgs);
      ...
      expect(common.prepareGaslessRun).toHaveBeenCalledWith(
        expect.objectContaining({ ...baseArgs, requiredRoles: ['holder'] }),
      );
      expect(trustvc.transferHolderGasless).toHaveBeenCalledWith(
        { titleEscrowAddress: '0xescrow' },
        smartAccountClient,
        { holderAddress: baseArgs.newHolder, remarks: baseArgs.remark },
    ...
    it('returns undefined and logs the message when prepareGaslessRun rejects with an Error', async () => {
      ...
      (common.prepareGaslessRun as MockedFunction<any>).mockRejectedValue(
        new Error('not the current holder'),
      );
    ...
    it('returns undefined and logs the raw value when the gasless call rejects with a non-Error', async () => {
      ...
      (trustvc.transferHolderGasless as MockedFunction<any>).mockRejectedValue('raw failure');
