...
import {
  acceptReturnedHandler,
  promptForInputs,
} from '../../../src/commands/obligation-escrow/accept-return-to-issuer';
...
const { burnPopulateTransaction, escrowAcceptReturned } = vi.hoisted(() => ({
  burnPopulateTransaction: vi.fn().mockResolvedValue({ to: '0xRegistry', data: '0xburn' }),
  escrowAcceptReturned: vi.fn(),
}));

vi.mock('../../../src/commands/helpers', () => ({
  connectToObligationEscrow: vi.fn().mockResolvedValue({
    // The escrow contract intentionally has no acceptReturned method: burning
    // a returned title happens on the registry, not the escrow.
    acceptReturned: escrowAcceptReturned,
  }),
  connectToObligationRegistry: vi.fn().mockResolvedValue({
    burn: { populateTransaction: burnPopulateTransaction },
  }),
...
describe('obligation-escrow/accept-return-to-issuer', () => {
  ...
  it('dry-run populates burn() on the registry contract, not the escrow', async () => {
    ...
    (trustvc.acceptReturnedObligationRegistry as MockedFunction<any>).mockResolvedValue({
      hash: '0xaccept-return',
      wait: vi.fn().mockResolvedValue({ hash: '0xaccept-return' }),
    });

    await acceptReturnedHandler({
    ...
    expect(burnPopulateTransaction).toHaveBeenCalledWith('0x1', '0xencrypted');
    expect(escrowAcceptReturned).not.toHaveBeenCalled();
    expect(trustvc.acceptReturnedObligationRegistry).toHaveBeenCalled();
  });
});
