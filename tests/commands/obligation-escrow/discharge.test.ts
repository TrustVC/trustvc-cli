...
import {
  dischargeHandler,
  promptForInputs,
} from '../../../src/commands/obligation-escrow/discharge';
...
vi.mock('../../../src/commands/obligation-escrow/runTx', () => ({
  runObligationEscrowTx: vi.fn().mockResolvedValue({ hash: '0xdischarge' }),
}));
...
describe('obligation-escrow/discharge', () => {
  ...
  it('dischargeHandler runs SDK via runObligationEscrowTx', async () => {
    const { runObligationEscrowTx } = await import('../../../src/commands/obligation-escrow/runTx');
    await dischargeHandler({
