...
import {
  endorseHandler,
  promptForInputs,
} from '../../../src/commands/obligation-escrow/endorse-transfer-owner';
...
vi.mock('../../../src/commands/obligation-escrow/runTx', () => ({
  runObligationEscrowTx: vi.fn().mockResolvedValue({ hash: '0xendorse' }),
}));
...
vi.mock('../../../src/utils', async (importOriginal) => {
  ...
  return {
    ...actual,
    promptAddress: vi.fn().mockResolvedValue('0xEndorsee0000000000000000000000000000001'),
...
describe('obligation-escrow/endorse-transfer-owner', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('promptForInputs includes newBeneficiary', async () => {
    ...
    expect(result.newBeneficiary).toBe('0xEndorsee0000000000000000000000000000001');
  });

  it('endorseHandler calls runObligationEscrowTx with beneficiary params', async () => {
    const { runObligationEscrowTx } = await import('../../../src/commands/obligation-escrow/runTx');
    await endorseHandler({
      network: NetworkCmdName.Amoy,
      obligationRegistryAddress: '0xRegistry',
      tokenId: '0x1',
      newBeneficiary: '0xEndorsee0000000000000000000000000000001',
      key: '0xabc',
      maxPriorityFeePerGasScale: 1,
    });
    expect(runObligationEscrowTx as MockedFunction<any>).toHaveBeenCalledWith(
      expect.objectContaining({
        sdkParams: expect.objectContaining({
          newBeneficiaryAddress: '0xEndorsee0000000000000000000000000000001',
        }),
      }),
    );
  });
