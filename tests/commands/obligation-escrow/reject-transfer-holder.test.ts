...
import {
  ...
  promptForInputs,
} from '../../../src/commands/obligation-escrow/reject-transfer-holder';
...
vi.mock('../../../src/commands/obligation-escrow/runTx', () => ({
  runObligationEscrowTx: vi.fn().mockResolvedValue({ hash: '0xreject-transfer-holder' }),
}));
...
describe('obligation-escrow/reject-transfer-holder', () => {
  ...
  it('rejectTransferHolderHandler runs SDK via runObligationEscrowTx', async () => {
    const { runObligationEscrowTx } = await import('../../../src/commands/obligation-escrow/runTx');
    await rejectTransferHolderHandler({
