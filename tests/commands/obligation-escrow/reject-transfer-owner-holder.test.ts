...
import {
  rejectTransferOwnersHandler,
  promptForInputs,
} from '../../../src/commands/obligation-escrow/reject-transfer-owner-holder';
...
vi.mock('../../../src/commands/obligation-escrow/runTx', () => ({
  runObligationEscrowTx: vi.fn().mockResolvedValue({ hash: '0xreject-transfer-owner-holder' }),
}));
...
describe('obligation-escrow/reject-transfer-owner-holder', () => {
  ...
  it('rejectTransferOwnersHandler runs SDK via runObligationEscrowTx', async () => {
    const { runObligationEscrowTx } = await import('../../../src/commands/obligation-escrow/runTx');
    await rejectTransferOwnersHandler({
