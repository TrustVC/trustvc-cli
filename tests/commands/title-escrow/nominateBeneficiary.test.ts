...
describe('title-escrow/nominate-change-owner', () => {
  ...
  describe('nominateChangeOwnerHandler', () => {
    let nominateMock: MockedFunction<any>;
    ...
    beforeEach(async () => {
      ...
      nominateMock = trustvcModule.nominate as MockedFunction<any>;
    ...
    it('should successfully nominate change of owner and display transaction details', async () => {
      const mockArgs: any = {
        network: NetworkCmdName.Sepolia,
        tokenRegistryAddress: '0x1234567890123456789012345678901234567890',
        tokenId: '0xabcdef1234567890',
      ...
      nominateMock.mockResolvedValue({
      ...
      expect(nominateMock).toHaveBeenCalledWith(
    ...
    it('should handle nominate with remark and encryption key', async () => {
      const mockArgs: any = {
        network: NetworkCmdName.Sepolia,
        tokenRegistryAddress: '0x1234567890123456789012345678901234567890',
        tokenId: '0xabcdef1234567890',
        newBeneficiary: '0x0987654321098765432109876543210987654321',
        remark: 'Important nomination',
    ...
    it('should handle errors during nominate', async () => {
      const mockArgs: any = {
        network: NetworkCmdName.Sepolia,
        tokenRegistryAddress: '0x1234567890123456789012345678901234567890',
        tokenId: '0xabcdef1234567890',
      ...
      nominateMock.mockRejectedValue(new Error(errorMessage));

      const result = await nominateChangeOwnerHandler(mockArgs);

      expect(result).toBeUndefined();
      expect(nominateMock).toHaveBeenCalled();
    });

    it('should handle non-Error exceptions during nominate', async () => {
      const mockArgs: any = {
        network: NetworkCmdName.Sepolia,
        tokenRegistryAddress: '0x1234567890123456789012345678901234567890',
        tokenId: '0xabcdef1234567890',
      ...
      nominateMock.mockRejectedValue('String error message');
  ...
  describe('handler', () => {
    ...
    it('should successfully execute the complete nominate change owner flow', async () => {
      ...
      const trustvcModule = await import('@trustvc/trustvc');
      const nominateMock = trustvcModule.nominate as MockedFunction<any>;
      nominateMock.mockResolvedValue({
        hash: mockTransaction.transactionHash,
        wait: vi.fn().mockResolvedValue(mockTransaction),
      });
